...
public class CommerceSearchKeywordLibrary extends AbstractKeywordLibrary
{
	...
	public void createCategoryBoostRule(final String fieldName, final String fieldValue, final int boostStrength, final String categoryCode, String operator)
	{
		createBoostRule(fieldName, fieldValue, boostStrength, categoryCode, SolrBoostConditionOperator.valueOf(operator));
	}

	public void createGlobalBoostRule(final String fieldName, final String fieldValue, final int boostStrength, String operator)
	{
	...
	public void setInStock(final String productCode, final boolean value)
	{
		try
		{
			...
			for (final BaseStoreModel store : baseSiteService.getCurrentBaseSite().getStores())
			{
				...
				final StockLevelStatus stockLevelStatus = commerceStockService.getStockLevelStatusForProductAndBaseStore(product, store);
	...
	private FacetData<SearchStateData> findFacetInGlobalConfiguration(final String facetName)
	{
		...
		return (FacetData) CollectionUtils.find(facets, new Predicate()
		{
			...
			{
				FacetData<SearchStateData> data = (FacetData<SearchStateData>) o;
			...
			final String categoryCode)
	{
		try
		{
			FacetSearchStateData facetSearchStateData = new FacetSearchStateData();
	...
	public void toggleFacetVisibilityForACategory(final String facetName, String categoryCode)
	{
		try
		{
