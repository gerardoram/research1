    ...
    <c:forEach items="${insuranceQuoteReviews}" var="quoteReview">
        ...
        <div class="span-7 right">
            <c:choose>
                ...
                <c:when test="${cartData.insuranceQuote.state eq 'UNBIND'}">
                    <c:if test="${not empty cartData.insuranceQuote.formattedStartDate}">
                        <spring:theme code="checkout.multi.quoteReview.start.date" text="Start Date:" arguments="${cartData.insuranceQuote.formattedStartDate}"/>
                    </c:if>
                </c:when>
            </c:choose>
        </div>
