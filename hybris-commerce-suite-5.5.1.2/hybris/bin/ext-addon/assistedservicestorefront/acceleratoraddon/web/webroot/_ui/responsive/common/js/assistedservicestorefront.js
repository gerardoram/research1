...
function addASMHandlers() {
    ...
    if ($("#_asmBindForm").length) {
        ...
        if(customerId === "readonly" && cartId === "disabled"){
...
function checkCartIdFieldAndToggleBind(cartIdField) {
	if( !$(cartIdField).hasClass('placeholder') ) {
        if (!isNaN(cartIdField.value) && (cartIdField.value.length == 8)) {
...
function validateEmail(mailAddress) {
    return ($('<input>').attr({ type: 'email', required:'required' }).val(mailAddress))[0].checkValidity();
}
