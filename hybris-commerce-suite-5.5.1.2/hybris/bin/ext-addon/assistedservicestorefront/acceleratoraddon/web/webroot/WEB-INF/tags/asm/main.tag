                    ...
                    <c:otherwise>
						...
						<form action="${bindActionUrl}" method="post" class="asmForm" id="_asmBindForm">
							<fieldset>
								...
								<c:choose>
									<c:when test="${not empty cart.entries}">
									...
									<c:otherwise>
										...
										<div class="ASM_input_holder cartId">
                                            ...
                                            <label><spring:theme code="asm.emulate.cart.label"/></label>
	                                        <input name="cartId" type="text" value="" placeholder="${cartPlaceholder}" class="ASM-input" autocomplete="off"/>
	                                    </div>
									</c:otherwise>
								</c:choose>
								<input type="hidden" name="CSRFToken" value="${CSRFToken}">
                                ...
                                <c:choose>
                                    <c:when test="${emulatedUser.uid ne 'anonymous'}">
                                        <button type="submit" class="ASM-btn ASM-btn-bind-cart hidden"><spring:theme code="asm.emulate.cart.bind"/><span class="hidden-xs hidden-sm"><spring:theme code="asm.emulate.cart.bind.ending"/></span></button>
                                    </c:when>
                                    <c:otherwise>
                                        <button type="submit" class="ASM-btn ASM-btn-bind-cart hidden"><spring:theme code="asm.emulate.cart.bind"/><span class="hidden-xs hidden-sm"><spring:theme code="asm.emulate.customer.bind.ending"/></span></button>
                                    </c:otherwise>
                                </c:choose>
                            </fieldset>
						</form>
