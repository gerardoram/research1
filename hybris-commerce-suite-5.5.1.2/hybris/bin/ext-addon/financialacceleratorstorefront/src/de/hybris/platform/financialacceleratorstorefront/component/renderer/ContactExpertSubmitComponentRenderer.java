...
import de.hybris.platform.financialacceleratorstorefront.model.components.CMSContactExpertSubmitComponentModel;
 ...
 */
public class ContactExpertSubmitComponentRenderer<C extends CMSContactExpertSubmitComponentModel>
	...
	public void renderComponent(final PageContext pageContext, final C component) throws ServletException, IOException
	{
		...
		try
		{
			if (StringUtils.isNotBlank(agentStatus))
			{
				...
				pageContext.getSession().setAttribute(FinancialacceleratorstorefrontConstants.FINANCIAL_FIND_AGENT_FORM_ID, newSessionFormDataId);
				getSessionService().setAttribute(FinancialacceleratorstorefrontConstants.FINANCIAL_FIND_AGENT_FORM_ID, newSessionFormDataId);
				...
				final String content = getYformFacade().getInlineFormHtml(applicationId, formId,
						YFormDataActionEnum.EDIT, yfdd.getId(), getContactExpertPreprocessorStrategy(), contactExpertParams);
