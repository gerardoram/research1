...
public class FormulaInsertEndpointAdapterTest
{
	private FormulaInsertEndpointAdapter adapter;
	...
	public void setUp()
	{
		...
		adapter = new FormulaInsertEndpointAdapter();
	...
	public void testExecuteEndpointAdapter()
	{
		...
		Mockito.verify(atsFacade).createFormula(Mockito.any(AtsFormula.class));
	}

	@Test
	public void testReturnCorrectErrorCodeWhenEntityValidationError()
	{
		Mockito.doThrow(EntityValidationException.class).when(atsFacade).createFormula(Mockito.any(AtsFormula.class));
		...
		{
			Assert.assertEquals(FormulaErrorCode._101.getCode(), AbstractImportException.getErrorCode(e));
		}
	}
}
