...
package com.hybris.mobile.lib.b2b.provider;
...
import com.hybris.mobile.lib.b2b.sync.CatalogSyncService;
...
public class CatalogProvider extends ContentProvider {
    ...
    public boolean onCreate() {
        ...
        try {
            ...
            componentName = new ComponentName(getContext(), CatalogSyncService.class);
            ...
            if (StringUtils.isBlank(accountName) || StringUtils.isBlank(accountType)) {
                throw new IllegalArgumentException("You must provide the metadata " + getContext().getString(R.string.sync_account_name) + ", " + getContext().getString(R.string.sync_account_type) + " for " + this.getClass());
            }
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CatalogContract.Provider.CODE_DATA:
                nbRowsDeleted = mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseDataSimple.TABLE_NAME, null, null);
                nbRowsDeleted += mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseDataLinkGroup.TABLE_NAME, null, null);
                nbRowsDeleted += mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseSyncStatusGroup.TABLE_NAME, null, null);

                break;

            // Delete all data details
            case CatalogContract.Provider.CODE_DATA_DETAILS:
                nbRowsDeleted = mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseDataDetails.TABLE_NAME, null, null);
                nbRowsDeleted += mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseDataLinkGroup.TABLE_NAME, null, null);
                nbRowsDeleted += mDatabaseHelper.getWritableDatabase().delete(CatalogContract.DataBaseSyncStatusGroup.TABLE_NAME, null, null);
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CatalogContract.Provider.CODE_DATA_ID:
                if (selection != null && selectionArgs != null) {
                    nbRowsUpdated = mDatabaseHelper.getWritableDatabase().update(CatalogContract.DataBaseDataSimple.TABLE_NAME, values,
            ...
            case CatalogContract.Provider.CODE_DATA_DETAILS_ID:
                if (selection != null && selectionArgs != null) {
                    nbRowsUpdated = mDatabaseHelper.getWritableDatabase().update(CatalogContract.DataBaseDataDetails.TABLE_NAME, values,
                            selection, selectionArgs);
                } else {
                    nbRowsUpdated = mDatabaseHelper.getWritableDatabase().update(CatalogContract.DataBaseDataDetails.TABLE_NAME, values,
                            CatalogContract.DataBaseData.ATT_DATA_ID + "=?", new String[]
    ...
    private void insertOrUpdate(Uri uri, ContentValues values) {
        ...
        switch (URI_MATCHER.match(uri)) {
            ...
            case CatalogContract.Provider.CODE_DATA_ID:
                insertOrUpdateGeneric(uri, values, CatalogContract.DataBaseData.ATT_DATA_ID, CatalogContract.DataBaseDataSimple.TABLE_NAME);
                break;

            // Insert or Update a data details
            case CatalogContract.Provider.CODE_DATA_DETAILS_ID:
                insertOrUpdateGeneric(uri, values, CatalogContract.DataBaseData.ATT_DATA_ID, CatalogContract.DataBaseDataDetails.TABLE_NAME);
                break;
    ...
    private void insertOrUpdateGeneric(Uri uri, ContentValues values, String idName, String tableName) {
        try {
            Log.i(TAG, "Inserting the information for the item " + values.getAsString(idName));
            mDatabaseHelper.getWritableDatabase().insertOrThrow(tableName, null, values);
        } catch (SQLiteConstraintException e) {
            Log.i(TAG, "Item " + values.getAsString(idName) + " already exists, updating it");
