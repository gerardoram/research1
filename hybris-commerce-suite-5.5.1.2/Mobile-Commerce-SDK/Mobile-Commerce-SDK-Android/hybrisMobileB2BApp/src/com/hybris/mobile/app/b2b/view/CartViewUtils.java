 ...
 ******************************************************************************/
package com.hybris.mobile.app.b2b.view;
...
import com.hybris.mobile.app.b2b.utils.NumberUtils;
    ...
    public static void createCartSummary(View view, Cart cart, boolean enableItems) {
        ...
        if (cart != null) {
            if (cart.getEntries() != null && !cart.getEntries().isEmpty()) {
                ...
                // Display tax price
                if (cart.getTotalTax() != null && NumberUtils.convertToInt(cart.getTotalTax().getValue()) > 0) {
                ...
                if (cart.getTotalDiscounts() != null && NumberUtils.convertToDouble(cart.getTotalDiscounts().getValue()) > 0) {
                    if (cart.getAppliedOrderPromotions() != null && !cart.getAppliedOrderPromotions().isEmpty()) {
                        mCartSummaryPromotion.setVisibility(View.VISIBLE);
                        // Nb order Promotion
                        StringBuffer promotion = new StringBuffer();
                        for (Promotion orderPromotion : cart.getAppliedOrderPromotions()) {
                ...
                mCartSummaryItems.setText(B2BApplication.getContext()
