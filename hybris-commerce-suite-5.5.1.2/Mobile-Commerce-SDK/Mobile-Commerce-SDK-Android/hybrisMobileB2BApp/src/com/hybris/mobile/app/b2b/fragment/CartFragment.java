...
import com.hybris.mobile.app.b2b.activity.CheckoutActivity;
import com.hybris.mobile.app.b2b.adapter.CartProductListAdapter;
...
public class CartFragment extends Fragment implements ListViewSwipeableListener
{
	...
	public void onActivityCreated(Bundle savedInstanceState)
	{
		...
		mCartProductListAdapter = new CartProductListAdapter(getActivity(), new ArrayList<OrderProduct>(), mProductList,
				mCartRequestId);
	...
	public void populateCartContent(Cart cart)
	{
		...
		if (cart != null)
		{
			...
			if (cart.getEntries() != null && !cart.getEntries().isEmpty())
			{
				...
				{
					for (Promotion productPromotion : cart.getAppliedProductPromotions())
					{
						if (productPromotion.getConsumedEntries() != null)
						{
							for (PromotionEntry promotionEntry : productPromotion.getConsumedEntries())
							{
								...
								{
									if (promotionEntry.getOrderEntryNumber() == cart.getEntries().get(i).getEntryNumber())
									{
										cart.getEntries().get(i).setPromotion(productPromotion);
									}
								}
							}
						}
	...
	{
		super.onStop();
		B2BApplication.getContentServiceHelper().cancel(mCartRequestId);
	}
