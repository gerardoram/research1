...
public class PersistenceHelper {
        ...
        DataResponseCallBack dataResponseCallBack = new DataResponseCallBack() {
            ...
            public void onResponse(DataResponse dataResponse) {
                ...
                // Conversion error, we return the response with the error message
                    ...
                    handleGenericError(null, responseReceiver, dataResponse, dataConverterHelper, requestId);
                }
            }
     ...
     * @param headers             Call parameters headers
     * @param httpMethod          Http method: GET, POST, PUT, DELETE
     * @return Generic List of Class Type Response
     */
    public <T, Z> boolean executeForList(final ResponseCallback<List<T>, Z> responseReceiver,
                                         final DataConverter.Helper<T, Z> dataConverterHelper, boolean getCachedResult, final String requestId, final String url,
                                         Map<String, String> params, Map<String, String> headers, String httpMethod) {

        DataResponseCallBack dataResponseCallBack = new DataResponseCallBack() {

            @Override
            public void onResponse(final DataResponse dataResponse) {

                try {
                    final Response<List<T>> response;
                ...
                // Conversion error, we return the response with the error message
                    ...
                    handleGenericError(responseReceiver, null, dataResponse, dataConverterHelper, requestId);
                }
            }
     ...
     *
     * @param responseListReceiver List of Generic Data Type
     ...
     */
    private <Z, T> void handleGenericError(final ResponseCallback<List<T>, Z> responseListReceiver,
                                           final ResponseCallback<T, Z> responseReceiver, DataResponse dataResponse, DataConverter.Helper<T, Z> dataConverterHelper,
                                           String requestId) {
        ...
        try {
            ...
            if (mUiRelated) {
                new Handler(Looper.getMainLooper()).post(new Runnable() {
                    @Override
                    public void run() {
                        if (responseListReceiver != null) {
                            responseListReceiver.onError(response);
                        } else {
                            responseReceiver.onError(response);
                        }
                    }
