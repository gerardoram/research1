...
- (HYBStore *)sampleStore {
    NSDictionary *json = [self jsonAsDictionary:@"getStoreDetailsSampleResponse"];
    return [MTLJSONAdapter modelOfClass:[HYBStore class] fromJSONDictionary:json error:nil];
}
...
- (NSArray *)sampleStores {
    ...
    for (NSDictionary *json in productsAsJSON) {
        HYBStore *store = [MTLJSONAdapter modelOfClass:[HYBStore class] fromJSONDictionary:json error:nil];
        [result addObject:store];
    }
    NSAssert([result hyb_isNotBlank], @"Stores must exist.");
    HYBStore *s = [result firstObject];
    NSAssert([s hyb_isNotBlank], @"Each HYBStore must exist.");
    NSAssert([[s name] hyb_isNotBlank], @"Each HYBStore name must exist.");
    return result;
}
...
- (HYBOrder *)sampleOrder {
    ...
    [order buildCartFromData:fullJSON];
    return order;
}
...
- (HYBCategory *)createCategoryTreeFromJsonFile:(NSString *)jsonFileName {
    HYBCategory *result  = [HYBCategory categoryWithParams:[self jsonAsDictionary:jsonFileName]];
...
- (NSDictionary *)jsonAsDictionary:(NSString *)fileName {
    NSData *cartData = [self dataFromJson:fileName];
    NSDictionary *cartJSON = [NSJSONSerialization JSONObjectWithData:cartData options:kNilOptions error:nil];
    return cartJSON;
}

- (NSData *)dataFromJson:(NSString *)jsonFileName {
    NSString *cartFilePath = [[NSBundle mainBundle] pathForResource:jsonFileName ofType:@"json"];
    NSData *cartData = [NSData dataWithContentsOfFile:cartFilePath];
    return cartData;
}
