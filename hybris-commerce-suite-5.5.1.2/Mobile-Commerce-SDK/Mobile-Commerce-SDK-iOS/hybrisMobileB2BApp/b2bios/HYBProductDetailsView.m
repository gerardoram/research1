...
- (void)refreshWithProduct:(HYBProduct*)product {
    ...
    _productPriceLabel.text     = _currentProduct.formattedPrice;
    _summaryTextView.text       = _currentProduct.summary;
    
    _availabilityLabel.text     = [NSString stringWithFormat:NSLocalizedString(@"%d in stock", nil), [_currentProduct.stock integerValue]];
    ...
    _addToCartButton.enabled = ([product.stock intValue] > 0);
...
-(void)recursivePreselectionVariants {
    [self isChildOfVariant:_currentProduct.variants depth:0];
}
...
- (NSArray *)variantsAsMatrix {
    ...
    if ([[_currentProduct variants] hyb_isNotBlank]) {
        
        id parent = _currentProduct;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    HYBProductVariantOption *variantOption = [[_currentVariantsMatrix objectAtIndex:_currentProduct.variantDimensionsNumber - 1] objectAtIndex:lastDimensionRow];
...
- (void)prepareTabs {
    ...
    _tableContent = [NSArray arrayWithObjects:
                     ...
                     //second cell content
                     [NSDictionary dictionaryWithObjectsAndKeys:
                      [_currentProduct deliveryDetails],@"content",
...
- (void)calculateTotalPrice {
    ...
    if (quantity) {
        ...
        if ([_currentProduct isVolumePricingPresent]) {
          for (NSDictionary *priceDict in _currentProduct.volumePricingData) {
                
                BOOL selectPrice = NO;
                
                NSNumber *maxQuantity = priceDict[@"maxQuantity"];
                NSNumber *minQuantity = priceDict[@"minQuantity"];
                ...
                if(selectPrice) {
                    unitPrice = [priceDict[@"value"] doubleValue];
                    _productPriceLabel.text = priceDict[@"formattedValue"];
                    break;
                }
            ...
            unitPrice = [_currentProduct.price floatValue];
        }
        
        CGFloat totalPrice = unitPrice * [quantity intValue];
        _totalItemPrice.text = [NSString stringWithFormat:@"%.02f %@", totalPrice, _currentProduct.currencyIso];
...
- (void)configPricingPanelForProduct:(HYBProduct*)product {
    ...
    if ([product isVolumePricingPresent]) {
...
- (void)buildPriceMatrix {
    ...
    for (NSDictionary *priceDict in _currentProduct.volumePricingData) {
        ...
        NSString *formattedValue = priceDict[@"formattedValue"];
        
        NSString *maxQuantity = [priceDict[@"maxQuantity"] stringValue];
        NSString *minQuantity = [priceDict[@"minQuantity"] stringValue];
