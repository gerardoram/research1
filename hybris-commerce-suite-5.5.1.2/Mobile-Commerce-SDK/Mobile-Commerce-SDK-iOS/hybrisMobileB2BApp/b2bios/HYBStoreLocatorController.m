...
- (void)bridgeGeoObjectsAndStores:(NSArray*)rawStores {
    ...
    for (HYBStore *store in rawStores) {
        ...
        if([HYBGeolocatorTools validateLatitude:latitude andLongitude:longitude]) {
            ...
            NSDictionary *options = [NSDictionary dictionaryWithObjectsAndKeys:
                                     ...
                                     store.formattedAddress,HYBGEODESCRIPTION,
                                     store, @"fullHYBStore",
                                     nil];
...
- (void)doPerformSearch {
    ...
    // __weak HYBStoreLocatorController *weakSelf = self;
    
    [self.b2bService findStoreBySearchQuery:self.currentSearchQuery andExecute:^(NSArray *stores, NSError *error) {
        ...
        if (error) {
            ...
            /*
             ...
             tmpParams[SEARCH_COUNT]  = [NSString stringWithFormat:@"%d",self.b2bService.totalSearchResults];
             
             NSDictionary *params = [NSDictionary dictionaryWithDictionary:tmpParams];
             
             //create/update search results if needed
             if(!weakSelf.searchResultsBar) {
             weakSelf.searchResultsBar = [[HYBSearchResultsBar alloc] initWithFrame:CGRectMake(0, [_mainView.headerToolbar bottom], [_mainView bW], 000)];
             [weakSelf.view addSubview:weakSelf.searchResultsBar];
             }
             
             if (weakSelf.searchResultsBar) {
             [weakSelf.searchResultsBar updateSearchResultWithParams:params animated:YES];
             [weakSelf didUpdateSearchResultsBarFrameAnimated:YES];
             
             if(hasDYM) {
             //action
             weakSelf.searchResultsBar.searchDidYouMeanPanel.userInteractionEnabled = YES;
             UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(didYouMeanTapped)];
             [weakSelf.searchResultsBar.searchDidYouMeanPanel addGestureRecognizer:tapGesture];
             }
