...
- (void)viewDidLoad {
    ...
    NSString *baseDate = _order.placed;
    if(!baseDate) baseDate = _order.created;
    ...
    _mainView.deliveryAddressValue.text = _order.deliveryAddress.formattedAddressBreakLines;
    ...
    if(_mainView.orderSummaryView) {
        ...
        NSString *totalUnitCount = [NSString stringWithFormat:NSLocalizedString(@"xx_items", nil), _order.cart.totalUnitCount];
        _mainView.orderSummaryView.itemCount.text = totalUnitCount;
        
        _mainView.orderSummaryView.subtotalValue.text   = _order.cart.subTotalFormatted;
        _mainView.orderSummaryView.taxValue.text        = _order.cart.totalTaxFormatted;
        _mainView.orderSummaryView.shippingValue.text   = _order.deliveryMode.formattedValue;
        _mainView.orderSummaryView.orderTotalValue.text = _order.cart.totalPriceFormatted;

        if (_order.cart.orderDiscounts.intValue > 0) {
            _mainView.orderSummaryView.savingsValue.text = _order.cart.orderDiscountsFormattedValue;
            _mainView.orderSummaryView.savingsRecapTitle.text = _order.cart.orderDiscountsMessage;
...
- (NSInteger)tableView:(UITableView *)theTableView numberOfRowsInSection:(NSInteger)section {
    return [_order.cart.items count];
}
...
- (UITableViewCell *)tableView:(UITableView *)table cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __block HYBEntry *item = _order.cart.items[currentRow];
...
- (void)tableView:(UITableView *)theTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HYBEntry *item = (HYBEntry*)_order.cart.items[indexPath.row];
