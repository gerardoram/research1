...
- (NSString *)backdoorLogin:(NSString *)paramsAsString {
    ...
    if(params) {
        ...
        [[self backEndService] authenticateUser:username
                                       password:password
                                          block:^(NSString *errorMsgOrToken, NSError *error) {
                                              ...
                                              if (error) {
                                                  ...
                                                  [[self backEndService] costCentersForCurrentStoreAndExecute:^(NSArray *array, NSError *err) {
...
- (NSString *)backdoorNavigateTo:(NSString *)paramsAsString {
    ...
    if(paramsAsString) {
        ...
        if ([destination isEqualToString:@"DashboardActivity"]) {
            //open dashboard page
            [[self centerController] openDashboard];
            resultString = @"DashboardActivity";
        }
        
        else if ([destination isEqualToString:@"OrdersActivity"]) {
            //open orders page
            [[self centerController] openOrders];
            resultString = @"OrdersActivity";
        }
        
        else if ([destination isEqualToString:@"AccountActivity"]) {
            //open account page
            [[self centerController] openAccount];
            resultString = @"AccountActivity";
        }
        
        else if ([destination isEqualToString:@"CatalogActivity"]) {
            //open catalog page
            resultString = @"CatalogActivity";
            [[NSUserDefaults standardUserDefaults] removeObjectForKey:STORAGE_CURRENTLY_SHOWN_CATEGORY_KEY];
            [[NSUserDefaults standardUserDefaults] synchronize];
            [[self centerController] openCatalog];
        }
        
        else if ([destination isEqualToString:@"CheckoutActivity"]) {
            //open checkout page
            resultString = @"CheckoutActivity";
...
- (NSString *)backdoorGoProductdetail:(NSString *)paramsAsString {
    ...
    if(paramsAsString) {
        ...
        [[self backEndService] findProductById:productId
        ...
        resultString = @"ProductDetailActivity: loading product";
    } else {
