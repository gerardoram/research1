...
- (void)viewDidLoad {
    ...
    _currentCart = [self.b2bService currentCartFromCache];
    _cartItems = self.currentCart.items;
    ...
    [_mainView.deliveryAddressValue setText:self.order.deliveryAddress.formattedAddressBreakLines];
    ...
    if(_mainView.orderSummaryView) {
        ...
        [_mainView.orderSummaryView.subtotalValue setText:self.currentCart.subTotalFormatted];
        [_mainView.orderSummaryView.taxValue setText:self.currentCart.totalTaxFormatted];
        
        [_mainView.orderSummaryView.shippingValue setText:self.order.deliveryMode.formattedValue];
        
        [_mainView.orderSummaryView.orderTotalValue setText:self.currentCart.totalPriceFormatted];
        
        if (_currentCart.orderDiscounts.intValue > 0) {
            [_mainView.orderSummaryView.savingsValue setText:self.currentCart.orderDiscountsFormattedValue];
            [_mainView.orderSummaryView.savingsRecapTitle setText:self.currentCart.orderDiscountsMessage];
    ...
    DDLogDebug(@"Cart total items in confirmation are %d", self.order.cart.totalItems.intValue);
}
...
- (UITableViewCell *)tableView:(UITableView *)table cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    __block HYBEntry *item = [self.cartItems objectAtIndex:currentRow];
...
- (void)tableView:(UITableView *)theTableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HYBEntry *item = [self.cartItems objectAtIndex:indexPath.row];
    
    [self.b2bService findProductById:item.product.code
                           withBlock:^(HYBProduct *product, NSError *error) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HYBEntry *item = (HYBEntry*)[self.cartItems objectAtIndex:indexPath.row];
