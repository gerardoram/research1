...
- (void)viewDidLoad {
    ...
    if (!_mainView) {
        ...
        //triple tap settings opener
        UITapGestureRecognizer *tripleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showSettings)];
        tripleTap.numberOfTouchesRequired   = 1;
        tripleTap.numberOfTapsRequired      = 3;
        [_mainView addGestureRecognizer:tripleTap];
...
- (void)showSettings {
    ...
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"settingsKey", nil)
                                                    message:NSLocalizedString(@"settingsMessageKey", nil)
                                                   delegate:self
                                          cancelButtonTitle:NSLocalizedString(@"cancelKey", nil)
                                          otherButtonTitles:NSLocalizedString(@"okayKey", nil), nil];
    
    [alert setAlertViewStyle:UIAlertViewStyleLoginAndPasswordInput];
    
    //grab old data :
    
    NSString *host      = _b2bService.backEndHost;
    NSString *port      = _b2bService.backEndPort;
    NSString *catalog   = _b2bService.currentCatalogId;
    
    NSString *hostPort = host;
    if([port hyb_isNotBlank]) hostPort = [host stringByAppendingFormat:@":%@",port];
    
    UITextField *alertTextField1 = [alert textFieldAtIndex:0];
    alertTextField1.placeholder = @"Host:Port";
    alertTextField1.text = hostPort;
    alertTextField1.keyboardType = UIKeyboardTypeURL;
    alertTextField1.clearButtonMode = UITextFieldViewModeWhileEditing;
    [alertTextField1 becomeFirstResponder];
    
    UITextField *alertTextField2 = [alert textFieldAtIndex:1];
    alertTextField2.placeholder = @"Catalog";
    alertTextField2.text = catalog;
    alertTextField2.keyboardType = UIKeyboardTypeDefault;
    alertTextField2.clearButtonMode = UITextFieldViewModeWhileEditing;
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        NSString *hostPort  = [alertView textFieldAtIndex:0].text;
        NSString *catalog   = [alertView textFieldAtIndex:1].text;
        
        if([hostPort hyb_isNotBlank] && [catalog hyb_isNotBlank]) {
            NSString *host = hostPort;
            NSString *port = nil;
            
            if ([hostPort rangeOfString:@":"].location != NSNotFound) {
                //has port
                NSArray *chunk = [host componentsSeparatedByString:@":"];
                host = [chunk firstObject];
                port = [chunk lastObject];
            }
            
            [_b2bService setBackEndHost:host];
            [_b2bService setBackEndPort:port];
            [_b2bService setCurrentCatalogId:catalog];
        } else {
            //alert problem
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"warningKey", nil)
                                                            message:NSLocalizedString(@"hostAndCatalogRequired", nil)
                                                           delegate:self
                                                  cancelButtonTitle:NSLocalizedString(@"cancelKey", nil)
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (currentUserId) {
        ...
        [_b2bService autoLoginUser:currentUserId block:^(NSString *errorMsgOrToken, NSError *error){
            if(error) {
...
- (void)loginWithUser:(NSString *)user pass:(NSString *)pass {
    ...
    [self.b2bService authenticateUser:user
                             password:pass
                                block:^(NSString *errorMsgOrToken, NSError *error) {
