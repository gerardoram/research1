...
- (id)initWithBackEndService:(id<HYBBackEndFacade>)backEndService {
...
- (void)setupPayload {
    ...
    _footerDict = [NSDictionary dictionaryWithObjectsAndKeys:
                   [NSLocalizedString(@"sidebar_logout", nil) uppercaseString],MENU_TITLE,
...
- (void)viewDidLoad {
    ...
    //footer
    _mainView.footerView = [self footerViewWithParams:_footerDict inFrame:CGRectMake(0, 0, [_mainView.menuTableView fW], _menuHeight)];
...
- (UIView*)footerViewWithParams:(NSDictionary*)params inFrame:(CGRect)frame {
    ...
    //centering
    CGFloat nudge = ([self maxWidthLeftDrawer]/2 - totalWidth/2)/2;
