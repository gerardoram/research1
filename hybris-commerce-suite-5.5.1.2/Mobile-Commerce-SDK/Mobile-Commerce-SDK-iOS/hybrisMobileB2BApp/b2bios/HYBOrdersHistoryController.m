...
- (void)doLoad {
    ...
    NSString *currentUserId = [[self.b2bService userStorage] objectForKey:LAST_AUTHENTICATED_USER_KEY];
    
    [self.b2bService retrieveOrdersForUser:currentUserId
                                withParams:sortDict
...
- (void)processNewOrders:(NSArray *)newOrders {
    ...
    if (!newOrders || [newOrders count] == 0) {
        ...
        if ([self.b2bService currentPage] == 0 && [_orders count] == 0) {
        ...
        if ([newOrders count] < [self.b2bService pageSize]) {
            _allOrdersLoaded = YES;
        }
    ...
    if (forceReload) {
        ...
        for (HYBOrder *newOrder in newOrders) {
            add = YES;
            for (HYBOrder *oldOrder in _orders) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HYBOrder *order = _orders[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.b2bService findOrderByCode:selectedOrder.code andExecute:^(HYBOrder *order, NSError *error) {
        ...
        if(error) {
            ...
            //fill useful infos
            order.placed        = selectedOrder.placed;
            order.total         = selectedOrder.total;
            order.totalValue    = selectedOrder.totalValue;
