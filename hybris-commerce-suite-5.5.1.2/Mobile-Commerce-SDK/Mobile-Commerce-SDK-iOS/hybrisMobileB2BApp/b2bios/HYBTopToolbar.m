...
-(void)setup {
    ...
    self.rightItems = [NSArray arrayWithObjects:_searchButtonView, _cartBadgeView,_feedbackButtonView, nil];
...
- (void)feedbackButtonTapped {
    ...
    if ([BITHockeyManager sharedHockeyManager].authenticator) {
        [[BITFeedbackManager alloc] showFeedbackListView];
    }
...
- (void)displaySearch:(BOOL)display animated:(BOOL)animated {
    ...
    if(animated) {
        [self setRightItemsViewHidden:YES completion:^() {
            [weakSelf swapDisplay:display];
        }];
    } else {
...
- (void)swapDisplay:(BOOL)display {
    if(display) {
        ...
        self.rightItems = [NSArray arrayWithObjects:_searchButtonView, _cartBadgeView, nil];
        [self clearSearch];
    }
}
