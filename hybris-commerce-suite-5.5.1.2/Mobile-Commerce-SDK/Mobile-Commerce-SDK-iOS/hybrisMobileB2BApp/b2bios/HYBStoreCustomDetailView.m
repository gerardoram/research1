...
@property (nonatomic) HYBStore *store;
...
- (void)setupWithStore:(HYBStore *)store {
    ...
    _storeAddressL1Label.text = [NSString stringWithFormat:@"%@, %@", _store.line1, _store.line2 ];
    
    _storeAddressL2Label.text = [NSString stringWithFormat:@"%@, %@", _store.town, _store.postalCode];
    
    _phoneLabel.text = _store.phone;
...
- (void)setupHours {
    ...
    for (NSDictionary *day in _store.openingHours) {
...
- (UIView*)viewForDay:(NSDictionary*)day {
    ...
    BOOL isClosed = [day[@"closed"] boolValue];
    ...
    if(isClosed) {
        ...
        NSDictionary *openDict = day[@"openingTime"];
        NSString *openFormattedString = openDict[@"formattedHour"];
        
        NSDictionary *closeDict = day[@"closingTime"];
        NSString *closeFormattedString = closeDict[@"formattedHour"];
