//
//  HYBOrderPromotion.m
// [y] hybris Platform
//
// Copyright (c) 2000-2015 hybris AG
// All rights reserved.
//
// This software is the confidential and proprietary information of hybris
// ("Confidential Information"). You shall not disclose such Confidential
// Information and shall use it only in accordance with the terms of the
// license agreement you entered into with hybris.
//


#import "HYBOrderPromotion.h"

#import "NSValueTransformer+MTLPredefinedTransformerAdditions.h"

@implementation HYBOrderPromotion


+ (instancetype)orderPromotionWithParams:(NSDictionary *)params {
    
    NSError *error = nil;
    
    HYBOrderPromotion *orderPromotion = [MTLJSONAdapter modelOfClass:[HYBOrderPromotion class]
                                                                fromJSONDictionary:params
                                                                             error:&error];
    
    if (error) {
        NSLog(@"Couldn't convert JSON to HYBOrderPromotion models: %@", error);
        return nil;
    }
    
    return orderPromotion;
}

+ (NSDictionary *)JSONKeyPathsByPropertyKey {
    return @{
             @"promotionDescription"    : @"description",
             @"promotion"               : @"promotion"
             };
}

//dicts
+ (NSValueTransformer *)promotionJSONTransformer {
    return [NSValueTransformer mtl_JSONDictionaryTransformerWithModelClass:[HYBPromotion class]];
}

@end
