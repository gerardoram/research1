...
describe(@"PresentHYBCart", ^{
    __block NSDictionary *json;
    __block HYBCart *cart;
    __block HYBEntry *cartItem;
    
    __block HYBEntry *firstEntry;
    ...
    it(@"should save and retrieve the whole cart to archive", ^{
        ...
        HYBEntry *retrievedItem = [NSKeyedUnarchiver unarchiveObjectWithData:loadedObject];
    ...
    it(@"should check new HYBCart object", ^{
        ...
        expect(cart.type).to.equal(@"cartWsDTO");
        ...
        expect(cart.deliveryCode).to.beFalsy();
    ...
    it(@"should check HYBCart potentialProductPromotions", ^{
        ...
        HYBProductPromotion *productPromotion = [cart.potentialProductPromotions firstObject];
        expect(productPromotion).to.beTruthy();
        expect(productPromotion.promotionDescription).to.equal(@"Buy 2 Power Drills, Get 1 Free");
        
        expect(productPromotion.consumedEntries).to.beTruthy();
        expect([productPromotion.consumedEntries count]).to.equal(1);
        
        HYBConsumedEntry * consumedEntry = [productPromotion.consumedEntries firstObject];
        ...
        expect(promotion.promotionDescription).to.equal(@"Buy 2 Power Drills, Get 1 Free");
    ...
    it(@"should check the paymentType in the cart", ^{
        expect(cart.paymentType).to.beTruthy();
        
        HYBPaymentType *paymentType = cart.paymentType;
        
        expect(paymentType.code).to.equal(@"ACCOUNT");
        expect(paymentType.displayName).to.equal(@"Account Payment");
    });
    ...
    it(@"should check HYBCart deliveryOrderGroups", ^{
        ...
        HYBDeliveryOrderGroup *deliveryOrderGroup = [cart.deliveryOrderGroups firstObject];
    ...
    it(@"should partially check one HYBEntry from HYBDeliveryOrderGroup of the cart", ^{
        ...
        HYBEntry *entry = [deliveryOrderGroup.entries firstObject];
    ...
    it(@"should check HYBCart appliedOrderPromotions", ^{
        ...
        HYBOrderPromotion *orderPromotion = [cart.appliedOrderPromotions firstObject];
        expect(orderPromotion).to.beTruthy();
        expect(orderPromotion.promotionDescription).to.equal(@"You saved $16.23 for spending over $500.00");
        ...
        expect(promotion.promotionDescription).to.equal(@"3% off orders over $500");
        expect(promotion.endDate).to.equal(@"2099-01-01T00:00:00");
    ...
    it(@"should check HYBCart appliedProductPromotions", ^{
        ...
        HYBProductPromotion *productPromotion = [cart.appliedProductPromotions firstObject];
        expect(productPromotion).to.beTruthy();
        expect(productPromotion.promotionDescription).to.equal(@"You get 1 Power Drill free for purchasing 2 Power Drills");
    ...
    it(@"should check one HYBEntry from the cart", ^{
        ...
        expect(firstEntry.discountMessage).to.equal(@"You get 1 Power Drill free for purchasing 2 Power Drills");
    });
    ...
    it(@"should check the product from the first HYBEntry in the cart", ^{
        ...
        //HYBStock check
        expect(product.productStock).to.beTruthy();
        expect(product.productStock.stockLevel).to.equal(491);
        expect(product.productStock.stockLevelStatus).to.equal(@"inStock");
